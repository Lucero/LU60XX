...
GPIO_ERR_NOT_SUPPORT = -1    # Lucero not support
...
def GPIO_SetInput(DevHandle,PinMask,PuPd):
    return Lucero.GPIO_SetInput(DevHandle,PinMask,PuPd)
# 将GPIO设置为输出模式
def GPIO_SetOutput(DevHandle,PinMask,PuPd):
    return Lucero.GPIO_SetOutput(DevHandle,PinMask,PuPd)
# 将GPIO设置为开漏模式（可做双向引脚）
def GPIO_SetOpenDrain(DevHandle,PinMask,PuPd):
    return Lucero.GPIO_SetOpenDrain(DevHandle,PinMask,PuPd)
# 控制GPIO输出高电平或者低电平
def GPIO_Write(DevHandle,PinMask,PinValue):
    return Lucero.GPIO_Write(DevHandle,PinMask,PinValue)
# 读取GPIO引脚状态
def GPIO_Read(DevHandle,PinMask,pPinValue):
    return Lucero.GPIO_Read(DevHandle,PinMask,pPinValue)
